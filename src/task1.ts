...
function percent(total: number, a: number): number {
    if (total === 0) {
...
function toDecimal(value: string, system: number): number {
...
function fromDecimal(value: number, system: number): string {
...
function calculateBinary(a: string, b: string, operation: (x: number, y: number) => number): string {
...
function rootBinary(a: string): string {
    const decimalA = toDecimal(a, binarySystem);
...
function percentBinaryHex(a: string, b: string, system: number): string {
...
function calculateHex(a: string, b: string, operation: (x: number, y: number) => number): string {
...
// ф-ция для вычисления корня с одним параметром
function rootHex(a: string): string {
    const decimalA = toDecimal(a, hexSystem);
    const resultDecimal = root(decimalA);
    return fromDecimal(resultDecimal, hexSystem);
}
